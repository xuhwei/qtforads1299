...
MainWindow::~MainWindow()
{
    if(tcp->startBoard){
        ...
        data_send.append(0xaa);
        data_send.append(0x06);
...
void MainWindow::getPort(){
    port = (quint16)ui->Port_lineEdit->text().toInt(nullptr,10);
...
void MainWindow::run_m(){
    ...
    if(!tcp->startBoard){
        ...
        /**************   生成模拟正弦波并画图*******************/
            ...
            for(int i = 0; i<channel_number; ++i){
                for(int j = 0; j< per_channel_number ;j++){
                    double y = 100.0*sin(2*PI*0.01*(i+1)*((double)j))+100.0;
                    data[i][j] = y;
                }
            }
        ...
        while(running){
             if(tcp->data_from_wifi.size() > per_channel_number*channel_number){
                 ...
                 for(int i=0; i<per_channel_number;i++){
                     for(int channel = 0; channel<channel_number; ++channel){
                         data[channel][i] = (double) tcp->data_from_wifi.front();
    ...
    if(tcp->startBoard){
        ...
        if(command.size()>1){
            for(int i = 0; i<command.size(); ++i){
                bool ok;
                c_send.append(command[i].toInt(&ok,16));
            }
